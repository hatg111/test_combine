...
<div class="container">
	...
	<br> <span id="ment">카테고리 별, 점수별로 맛집 랭킹을 확인해보세요!</span>
	<form class="rangebystar" name="frm" method="POST">
	...
	<div class="restlist">
		...
		<ul class="event-list">
			...
			<c:forEach var="storelist" items="${storelist}">
				<li class="rankinglist" storeid="${storelist.storeid}"><img
					class="storeimg" src="${storelist.img}" />
					<div class="info">
						<div id="floatul">
							<ul>
								<li id="jhap">종합<span class="${storelist.star} starrate">
										<c:out value="${storelist.star}" />
								</span></li>
								<li>맛<span class="starrate"> <c:out
											value="${storelist.tastestar}" /></span> 가성비<span class="starrate">
										<c:out value="${storelist.costefstar}" />
								</span> 서비스<span class="starrate"> <c:out
											value="${storelist.servicestar}" /></span></li>
							</ul>
						</div>
